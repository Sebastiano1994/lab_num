...
using namespace std;
	...
	while(i<nstep){
	Shuffle();
	if(i % 10==0) {ending[ind]=min(q,npop); ending[ind+1]=beta; ind+=2;}
	beta+=.1;
	i++;}
	
	//print all lenght in dati_k.dat (k referring to k-th core)
	ofstream ex;
	string title;
	stringstream srank;
	srank <<"dati_"<<rank<<".csv";
	title = srank.str();
	ex.open(title);
	for(int i=0; i<maximum; i+=2) ex<<setw(6)<<ending[i]<<endl;
	ex.close();
	...
	double data[size];
	MPI_Gather(&ending[maximum-2],1,MPI_REAL8,data,1,MPI_REAL8,0, MPI::COMM_WORLD);//maximum-2 is the last lenght, maximum-1 last beta
	if(rank==0){
	ex.open("sums.csv");
	for(int i=0; i<size; i++) {ex<<i<<",   "<<setw(6)<<data[i]<<endl; cout<<data[i]<<endl;}
	ex.close();}
...
//Input function (initialize):
	...
	//function to find index referred to minimum element (in an array)
