...
const double fatt_k = pow(sigma/sec,2)*mass;
const double fatt_t = kb/eps;
const double fatt_P =  pow(sigma*sigma*sigma/eps,-1);

const int nbins=100;	//number of bins for g(r) histogram
double g[nbins]; 	//histogram array
double bin_size;	//size for each bin
double vtail, ptail;
double pi = M_PI;
