...
//Input function (initialize):	
	...
	if(G1<30){
	//second side
	G2 = (int)abs(rnd.Rannyu()*(30-G1));
	for(int i=G1; i<G2+G1; i++){
	coord[i][0]=1;
	...
	//thrid side
	if(G1+G2<30){
	G3 = (int)abs(rnd.Rannyu()*(30-G1-G2));
	for(int i=G2+G1; i<G3+G2+G1; i++){
	coord[i][1]=1;
	coord[i][0]=abs(rnd.Rannyu());
	cout<<i<<")"<<coord[i][0]<<"; "<<coord[i][1]<<endl;}
	cout<<G3<<endl;}
	...
	if(G1+G2+G3<30){
	G4 = 30-G1-G2-G3;
	for(int i=G3+G2+G1; i<G4+G3+G2+G1; i++){
	coord[i][0]=0;
	coord[i][1]=abs(rnd.Rannyu());
	cout<<i<<")"<<coord[i][0]<<"; "<<coord[i][1]<<endl;}
	cout<<G4<<endl;
	...
	cout<<endl;
	cout<<"Random cities generated on a squared line"<<endl<<endl;
